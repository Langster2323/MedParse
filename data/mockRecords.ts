export type ReviewStatus = "Reviewed" | "Pending Review" | "In Progress" | "Flagged";
...
export interface MedicalRecord {
  ...
  // For the simulated upload mapping
  sampleFile?: string;
}
...
// Maps sample file names to specific records for the upload demo
export const sampleFileMapping: Record<string, string> = {
  "Sample — Primary Care Visit.pdf": "rec-001",
  "Sample — Medication Review.pdf": "rec-003",
  "Sample — Patient Summary.pdf": "rec-008",
};

export const getRecord = (id: string): MedicalRecord | undefined =>
  mockRecords.find((r) => r.id === id);

export const getStatusColor = (status: ReviewStatus) => {
  switch (status) {
    case "Reviewed":
